        ...
        when (pageNumber) {
            PAGE3 -> when (currentKey) {
                NORMAL_CATEGORY -> {
                    ...
                    Toast.makeText(context, categoryName + " 이(가) 관심 목록에 추가되었습니다.", Toast.LENGTH_SHORT).show()
                }
                INTEREST_CATEGORY -> {
                    ...
                    Toast.makeText(context, categoryName + " 이(가) 관심 목록에서 해제되었습니다.", Toast.LENGTH_SHORT).show()
                }
                UNINTEREST_CATEGORY -> {
            ...
            PAGE4 -> when (currentKey) {
                NORMAL_CATEGORY -> {
                    ...
                    button.setTextColor(Color.WHITE)
                    Toast.makeText(context, categoryName + " 이(가) 관심 없음 목록에 추가되었습니다.", Toast.LENGTH_SHORT).show()
                }
                UNINTEREST_CATEGORY -> {
                    SharedPreferenceUtil.set(context, categoryName, NORMAL_CATEGORY)
                    ...
                    Toast.makeText(context, categoryName + " 이(가) 관심 없음 목록에서 해제되었습니다.", Toast.LENGTH_SHORT).show()
                }
                INTEREST_CATEGORY -> {
                    Toast.makeText(context, message, Toast.LENGTH_SHORT).show()
                }
                ...
                UNINTEREST_CATEGORY -> {
                    ...
                    button.setBackgroundResource(R.drawable.circle_shape_white_blue)
                    button.setTextColor(ContextCompat.getColor(context, R.color.colorPrimary))
                    Toast.makeText(context, categoryName + " 이(가) 기본 상태로 변경 되었습니다.", Toast.LENGTH_SHORT).show()
                }
                INTEREST_CATEGORY -> {
